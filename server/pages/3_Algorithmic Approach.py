...
def cluster_data(st):
    ...
    st.write("""
            This section shows the results of running a clustering algorithm, known as Density-Based Spatial Clustering of Applications with Noise (DBSCAN), on device and manufacturer names in event and recall tables. To generate the clusters, the names were concatenated and preprocessed using TF-IDF and run against DBSCAN using cosine similarity as our string distance metric. 
            The model was configured such that distance between different values was very small to avoid grouping unlike names, so it is possible many clusters will consist of just 1 value.
            After the clusters were identified and the data was labeled, we generated a plot by mapping the vectors into a two-dimensional space via PCA to visualize the clusters. 

            Below, you can search for a cluster number and see the resulting names within the cluster. 
    """)
    path = 'model_objects/cluster'
    df, plot = a.get_cluster_objects(path)

    # show the total number of clusters
    unique_clusters = df['cluster'].nunique()
    st.metric('Number of Clusters (Unique Device and Manufacturer Names)', unique_clusters)
    # biggest_cluster = df['cluster'].value_counts().idxmax()
    # col1.metric('Largest Cluster Number', biggest_cluster)

    # prompt user for cluster id
    cluster = int(st.number_input('Enter Cluster Number', value=0))
    # get all items in  cluster
    get_cluster = df[df.cluster == cluster]
    st.dataframe(get_cluster[['cluster','device_name', 'manufacturer_name']])
...
def main():
    st.set_page_config(
        page_title="OpenFDA Medical Devices - Applied Algorithms",
        page_icon="⚕️",
        layout="wide"
    )
